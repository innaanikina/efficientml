...
from triton_kernels.quantize_kernel_asym import dequantize_rowwise_int4_asym_ref


def matmul_x16_w4_asym_ref(
    x: torch.Tensor,
    ...
    w = dequantize_rowwise_int4_asym_ref(w_packed, w_scales, w_zp, in_features).to(x.dtype)
    return x @ w.T


@triton.jit
def _matmul_x16_w4_asym_kernel(
    x_ptr, w_ptr, scale_ptr, zp_ptr, y_ptr,
    B, IN, OUT, PACK_IN,
    ...
    """
    Triton-кернел для X16 @ W4_asym^T.
    """
    ...
    zp = tl.load(zp_ptr + col_idx, mask=scale_mask, other=0).to(tl.float32)  # (BLOCK_N,)
    ...
    for k in range(0, IN, BLOCK_K):
        ...
        w_shifted = tl.reshape(wu.to(tl.int32), (BLOCK_N, BLOCK_K)).to(tl.float32) - zp[:, None]
        w_shifted = tl.where(k_idx[None, :] < IN, w_shifted, tl.zeros_like(w_shifted))

        acc += tl.dot(x_block, tl.trans(w_shifted.to(tl.float16)))
...
def matmul_x16_w4_asym(
    ...
    assert x.is_cuda and w_packed.is_cuda and w_scales.is_cuda and w_zp.is_cuda
    assert x.dtype == torch.float16
    assert w_packed.dtype == torch.int32
    assert w_zp.dtype == torch.int32
    ...
    _matmul_x16_w4_asym_kernel[grid](
        x_ptr=x, w_ptr=w_packed, scale_ptr=w_scales, zp_ptr=w_zp, y_ptr=y,
...
@triton.jit
def _matmul_x16_w4_asym_kernel_autotuned(
    x_ptr, w_ptr, scale_ptr, zp_ptr, y_ptr,
    B, IN, OUT, PACK_IN,
    BLOCK_M: tl.constexpr,
    BLOCK_N: tl.constexpr,
    ...
    scale_mask = col_idx < OUT
    scales = tl.load(scale_ptr + col_idx, mask=scale_mask, other=0.0).to(tl.float32)
    zp = tl.load(zp_ptr + col_idx, mask=scale_mask, other=0).to(tl.float32)
...
def matmul_x16_w4_asym_autotuned(
    x: torch.Tensor,
    w_packed: torch.Tensor,
    w_scales: torch.Tensor,
    w_zp: torch.Tensor,
    ...
    _matmul_x16_w4_asym_kernel_autotuned[grid](
        x, w_packed, w_scales, w_zp, y,
        B, IN, OUT, PACK_IN,
    )
    return y
